...
<%
	...
	List<Hostel> hostelList = userDao.getHostel((Hostel)session.getAttribute("hostel"));
	Hostel hostel = hostelList.get(0);
%>
...
<div class="main">
	...
	<h2><span>Register now</span></h2>
		...
		<spr:form action="update.htm" method="post" commandName="hostel">
			
			<spr:input placeholder="Full Name" name="Name" path="studentName" value="<%=hostel.getStudentName() %>" type="text" required=""/>
			<spr:input placeholder="Email" value="<%=hostel.getEmailId() %>" readonly="true" path="emailId"  type="email" required=""/> 
			<spr:input placeholder="Phone Number" value="<%=hostel.getContact() %>" name="phone" path="contact" type="text" required=""/>
			
					
			Gender:<spr:select  value="<%=hostel.getGender()%>" path="gender">
		<br><br>
	...
	<div class="well"> 
	      ...
	      <spr:input type="date" value="<%=hostel.getDateOfBirth() %>" class="form-control" id="exampleInputDOB1" path="dateOfBirth" placeholder="Date of Birth"/>
	   	</div>
	</div>
	...
	<spr:select path="bloodGroup" value="<% %>">
